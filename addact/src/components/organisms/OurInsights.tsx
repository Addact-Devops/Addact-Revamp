...
import { getOurInsights, getHomeOurInsightsTitle, type HomeResponse } from "@/graphql/queries/getOurInsights";
...
function InsightCard({ item }: InsightCardProps) {
    return (
        <Link href={item.link} target='_self' className='flex-1 min-w-[300px] max-w-full md:max-w-[calc(33.333%-14px)]'>
                ...
                <div className='px-5 py-[30px]'>
                    ...
                    {item.date !== "Unknown Date" && (
                        <div className='flex items-center gap-2'>
                            <p className='text-stone-950 text-base font-medium font-montserrat mb-0!'>{item.date}</p>
                            <span className='text-stone-950 text-base font-medium font-montserrat'>•</span>
                            <p className='text-stone-950 text-base font-medium font-montserrat'>{item.readTime}</p>
                        </div>
                    )}
                </div>
            </div>
