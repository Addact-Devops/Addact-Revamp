...
const DetailPageServices = ({ title = "Our Services", items, isCaraousl = false }: DetailPageServicesProps) => {
    ...
    const renderCard = (item: DetailPageServiceItem, index: number, isCarouselCard: boolean) => {
        return (
            <Link
                key={item.id}
                href={item.link.href}
                ...
                className={[
                    "group relative w-full max-w-[375px] cursor-pointer overflow-hidden rounded-[14px] border border-[#E5E5E5] bg-white focus:outline-none",
