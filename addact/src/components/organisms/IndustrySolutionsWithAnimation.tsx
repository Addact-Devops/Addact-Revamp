    ...
    >
      <div className="container">
        <div className="flex flex-col">
          ...
          <div className="desktop-grid hidden md:grid grid-cols-1 lg:grid-cols-3 gap-6">
            {cards.map((card, idx) => (
              <div
                key={idx}
                className={`group relative bg-white border border-zinc-200 border-l-[3px] md:border-l-[5px] border-l-[#3C4CFF] sm:py-8 sm:px-8 px-[30px] py-[40px] ${
                  animate ? "show" : ""
                }`}
                style={
                  animate
              ...
              >
                <h3 className="text-black text-[20px]! md:text-[27px]! 2xl:text-[30px]! mb-[30px]">
                  {card?.Title ?? ""}
                </h3>
          ...
          <div className="md:hidden">
              ...
              {chunkArray<SolutionCard>(cards, 2).map((group, i) => (
                <div key={i} className="space-y-[16px]">
                  {group.map((card, j) => (
                    <div
                      key={`${i}-${j}`}
                      className="relative group bg-white pb-15 border border-zinc-200 border-l-[3px] border-l-[#3C4CFF] p-[16px]"
                    >
                      <h3 className="text-black text-[17px]! md:text-[30px]! mb-3">
                        {card?.Title ?? ""}
                      </h3>
                      <RichText html={card?.Description ?? ""} />
                    </div>
            ...
            <div className="relative mt-[40px] h-[4px] flex items-center justify-center">
              {/* Center gray line */}
              <div className="absolute left-0 right-0 top-1/2 h-px bg-[#D9D9D9] -translate-y-1/2"></div>
