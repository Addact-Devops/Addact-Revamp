...
export type CareerFormErrors = Partial<
  Record<keyof CareerFormState | "resume" | "captcha", string>
>;
...
export const validateCareerForm = (
  ...
  if (!rawPhone) {
    ...
    if (!(globalRegex.test(normalized) || indianLocalRegex.test(normalized))) {
      errors.phone =
        "Please enter a valid mobile number (with country code or 10-digit Indian number).";
    }
  }
  ...
  if (!resumeFile) {
    ...
    errors.resume = "Resume file size must be 5MB or less.";
  }
