...
export default function StaticOurProcess() {
  ...
  useEffect(() => {
    ...
    if (current) {
      ...
      const dotRadius =
        (currentDotEl?.clientHeight ?? (isMobile ? 16 : 24)) / 2;

      const currentDotTop =
        current.offsetTop + current.offsetHeight / 2 - offsetFix;

      let nextDotTop: number;
      if (next) {
        const nextDotEl = next.querySelector("div[class*='rounded-full']");
        const nextDotRadius =
          (nextDotEl?.clientHeight ?? (isMobile ? 16 : 24)) / 2;
  ...
  return (
    <section
      className="container-main mb-[100px] my-[80px] lg:my-[100px] 2xl:my-[200px]"
      ref={containerRef}
    >
      <div>
              ...
              return (
                ...
                >
                  ...
                  <div className="block md:hidden w-full pl-[25px]">
                    ...
                    <div className="font-normal text-[18px] mb-[12px]">
                      {step.Title}
                    </div>
