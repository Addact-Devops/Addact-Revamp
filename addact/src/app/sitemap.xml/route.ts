...
export async function GET() {
  ...
  // Blogs
  const blogData = await getInitialBlogs();
  ...
  // Careers
  const careerData = await client.request<{
    careerDetails: { Slug: string }[];
  }>(GET_ALL_CAREER_SLUGS);
  const careerUrls = careerData.careerDetails
    .map((c) => c.Slug?.replace(/^\//, ""))
    .filter(Boolean);

  // Events
  const eventData = await getEventListPageData();
  const eventUrls = eventData.addactsEvents
    .map((e) => `events/${e.Slug?.replace(/^\//, "")}`)
    .filter(Boolean);
  ...
  const subServiceData = await client.request<{
    subServicePages: { Slug: string }[];
  }>(GET_ALL_SUB_SERVICE_SLUGS);
  ...
  const subServiceUrls = subServiceData.subServicePages
    ...
    .filter(Boolean)
    .map((slug) => {
      const keyword = platformKeywords.find((k) =>
